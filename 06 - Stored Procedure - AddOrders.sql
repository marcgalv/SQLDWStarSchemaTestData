...
BEGIN
	...
	IF NOT EXISTS(SELECT TOP 1 * FROM [dbo].[Orders])
		BEGIN
	...
	ELSE
		BEGIN
			...
			SELECT @LastestCatA = MAX([CatA]) FROM [dbo].[Orders]
			SELECT @LastestCatB = MAX([CatB]) FROM [dbo].[Orders] WHERE [CatA] = @LastestCatA
			SELECT @LastestCatC = MAX([CatC]) FROM [dbo].[Orders] WHERE [CatA] = @LastestCatA AND [CatB] = @LastestCatB
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					INSERT INTO [dbo].[Orders] ([CatA], [CatB], [CatC], [DescA], [DescB]) VALUES(@CatAInsert, @CatBInsert, @CatCInsert, RIGHT(CONVERT(VARCHAR(255), NEWID()), 5), RIGHT(CONVERT(VARCHAR(255), NEWID()), 5)) 

					SET @count = @count + 1
					IF @count = @intQtdOrders + 1
